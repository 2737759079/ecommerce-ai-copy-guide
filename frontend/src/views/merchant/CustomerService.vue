<template>
  <div class="h-[calc(100vh-3rem)] flex gap-4 fade-in-up">
    <!-- 会话列表 -->
    <div class="w-80 card flex flex-col overflow-hidden shrink-0">
      <div class="px-5 py-4 border-b border-primary-light/50 flex items-center justify-between">
        ...
        <span v-if="totalUnread" class="badge badge-red text-xs">{{ totalUnread }} 条未读</span>
      </div>
      <div class="flex-1 overflow-y-auto p-3 space-y-2 bg-page">
        ...
        >
          <div class="flex items-center space-x-3">
            <img v-if="t.user_avatar_url" :src="normalizeImage(t.user_avatar_url)" class="w-10 h-10 rounded-full object-cover border border-primary-light" />
            <div v-else class="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-accent-blue flex items-center justify-center text-white text-sm font-bold">
              {{ (t.user_nickname || 'U').charAt(0).toUpperCase() }}
            </div>
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between">
                <span class="font-medium text-gray-800 truncate">{{ t.user_nickname }}</span>
                <span v-if="t.unread_count" class="badge badge-red text-xs ml-2">{{ t.unread_count }}</span>
              </div>
    ...
    <div class="flex-1 card flex flex-col overflow-hidden min-w-0">
      ...
      <template v-else>
        <div class="px-6 py-4 border-b border-primary-light/50 flex items-center justify-between bg-white shrink-0">
          <div class="flex items-center space-x-3">
            <img v-if="activeThread?.user_avatar_url" :src="normalizeImage(activeThread.user_avatar_url)" class="w-10 h-10 rounded-full object-cover border border-primary-light" />
            <div v-else class="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-accent-blue flex items-center justify-center text-white text-sm font-bold">
        ...
        <div ref="box" class="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-page">
          <div v-for="(msg, idx) in messages" :key="msg.id || idx" :class="msg.sender_role === 'merchant' ? 'text-right' : 'text-left'" class="fade-in-up">
            <div class="flex items-end space-x-2" :class="msg.sender_role === 'merchant' ? 'flex-row-reverse space-x-reverse' : ''">
              <div
            ...
            <p class="text-xs text-gray-400 mt-1" :class="msg.sender_role === 'merchant' ? 'pr-11' : 'pl-11'">{{ formatTime(msg.created_at) }}</p>
          </div>
        </div>

        <div class="p-4 bg-white border-t border-primary-light/50 shrink-0">
          <div class="flex items-center space-x-3">
            <FormInput v-model="input" @keydown.enter="sendReply" type="text" placeholder="请输入回复内容" :icon="ChatBubbleBottomCenterTextIcon" class="flex-1" input-class="rounded-xl py-3" />
            <button
              @click="sendReply"
...
import { ChatBubbleLeftRightIcon, CubeIcon, PaperAirplaneIcon, ChatBubbleBottomCenterTextIcon, InboxIcon } from '@heroicons/vue/24/outline'
...
const activeThread = computed(() => threads.value.find(t => t.user_id === activeUserId.value))
...
async function loadThreads() {
  try {
    const res = await api.get('/cs/merchant/threads')
...
async function loadMessages(userId) {
  try {
    const res = await api.get(`/cs/merchant/threads/${userId}`)
...
async function markRead(list) {
  const unread = list.filter(m => m.sender_role === 'user' && !m.is_read)
  for (const m of unread) {
    try {
      await api.patch(`/cs/messages/${m.id}/read`)
    } catch (e) {
      // ignore
    }
  }
...
async function sendReply() {
  ...
  input.value = ''
  try {
    await api.post(`/cs/merchant/threads/${activeUserId.value}/reply`, { content: text })
